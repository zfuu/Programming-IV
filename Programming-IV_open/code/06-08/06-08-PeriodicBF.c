...
void ic2_DrawFrame (void) {
  static int loopmax = 20; // loopmax回ごとに１回色を変える
  ...
  // (4) 物体の描画
  if (loopcounter - loopmax/2.0 >= 0) 
    ic2_OpenGLLogo((loopcounter - loopmax/2.0)/(loopmax/2.0));
  else
    ic2_OpenGLLogo((loopmax/2.0 - loopcounter)/(loopmax/2.0));
